...
const formalityLevels = [
  {
    ...
    label: "Formal",
    icon: "🎩",
    description: "Business, academic, official",
  },
  {
    value: "neutral",
    label: "Neutral",
    icon: "😊",
    description: "Standard, everyday conversation",
  },
  {
    value: "informal",
    label: "Casual",
    icon: "😎",
    description: "Friendly, relaxed, slang",
  },
];
...
export const Translator = ({ initialMode = "type" }) => {
  ...
  const fetchUserPreferences = async () => {
    try {
      const response = await axios.get(
        `${API_URL}/user/preferences?userId=${userId}`
      );
  ...
  const detectLanguage = async (text) => {
    ...
    try {
      ...
      if (detected && detected !== sourceLang) {
        ...
        showNotification(
          `Language detected: ${getLanguageName(detected)}`,
          "info"
        );
        recordAnalytics("language_detected", {
          detected,
  ...
  const playTranslatedAudio = async (text, langCode = targetLang) => {
    ...
    try {
      ...
      if (audioContent) {
        ...
        audio.onplay = () =>
          recordAnalytics("tts_played", { language: langCode });
        audio.onerror = () =>
  ...
  const toggleFavorite = (entry) => {
    ...
    recordAnalytics("favorite_toggled", {
      ...
      >
        ...
        <Typography
          ...
          sx={{
            mt: 1,
            fontStyle: "italic",
            ...getTextStyle(message.translation, message.targetLang),
          }}
        >
          {message.translation}
        </Typography>
      ...
      <Container maxWidth="lg" sx={{ py: 3 }}>
        ...
        <Box sx={{ mb: 3 }}>
          <Tabs
            ...
            sx={{ mb: 3 }}
          >
            <Tab icon={<TranslateIcon />} label="Translate" value="translate" />
            <Tab
              icon={<ChatIcon />}
              label="Conversation"
              value="conversation"
            />
            <Tab icon={<SchoolIcon />} label="Learning" value="learning" />
          </Tabs>
                ...
                <Card elevation={2}>
                  <CardContent>
                    <Stack
                      ...
                      mb={2}
                    >
                      <Typography variant="h6">Conversation Mode</Typography>
                      <Box sx={{ display: "flex", gap: 1 }}>
                        <Button
                          size="small"
            ...
            <Stack spacing={3} sx={{ mt: 1 }}>
              <Box>
                ...
                <Slider
                  value={speechSpeed}
                  onChange={(e, value) =>
                    handleSettingsChange("speechSpeed", value)
                  }
                  min={0.5}
                  max={2.0}
                  step={0.1}
                ...
                <FormControlLabel
                  control={
                    <Switch
                      checked={autoPlayTranslations}
                      onChange={(e) =>
                        setAutoPlayTranslations(e.target.checked)
                      }
                    />
                  }
                  label="Automatically play translation audio"
                />
              </Box>
              <Box>
                <Typography variant="subtitle1" gutterBottom>
                ...
                <FormControlLabel
                  ...
                  label="Save translations to history"
                />
              </Box>
            </Stack>
          </DialogContent>
